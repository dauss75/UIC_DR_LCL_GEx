...
colnames(rawBatch.sorted)<-sampleInfo$V2 
colnames(normalizedBatch.sorted)<-sampleInfo$V2

rawBatch.sorted<-rawBatch.sorted[,mixedorder(colnames(rawBatch.sorted), decreasing=FALSE)]
normalizedBatch.sorted<-normalizedBatch.sorted[,mixedorder(colnames(normalizedBatch.sorted), decreasing=FALSE)]
...
# eset.filtered.normalizedData<-as.data.frame(eset.filtered.normalizedData)
# rownames(eset.filtered.normalizedData)<-eset.filtered.normalizedData$V1; eset.filtered.normalizedData$V1<-NULL
...
# 
# normalizedDataMatrix<-normalizedDataMatrix[,mixedorder(colnames(normalizedDataMatrix), decreasing=FALSE)]
rm(eset.filtered.normalizedData)
...
normDataMatrix<-apply(normalizedDataMatrix,2,as.numeric); rownames(normDataMatrix)<-rownames(normalizedDataMatrix)

S1.expr<-data.frame(apply(normDataMatrix[,S1],1,mean));  colnames(S1.expr)<-S1[1]
SN1.expr<-data.frame(apply(normDataMatrix[,SN1],1,mean));  colnames(SN1.expr)<-SN1[1]

S2.expr<-data.frame(apply(normDataMatrix[,S2],1,mean));  colnames(S2.expr)<-S2[1]
SN2.expr<-data.frame(apply(normDataMatrix[,SN2],1,mean));  colnames(SN2.expr)<-SN2[1]

S3.expr<-data.frame(apply(normDataMatrix[,S3],1,mean));  colnames(S3.expr)<-S3[1]
SN3.expr<-data.frame(apply(normDataMatrix[,SN3],1,mean));  colnames(SN3.expr)<-SN3[1]

drops=c(S1, S2, S3, SN1, SN2, SN3)
normalizedDataMatrix<-normDataMatrix[ , !(colnames(normDataMatrix) %in% drops)]

normalizedDataMatrix2<-cbind(normalizedDataMatrix, S1.expr, SN1.expr, S2.expr, SN2.expr, S3.expr, SN3.expr)
normalizedDataMatrix2<-normalizedDataMatrix2[,mixedorder(colnames(normalizedDataMatrix2), decreasing=FALSE)]
targets2<-readTargets(paste(PhenotypeDir,"target2.txt", sep=''))
colnames(normalizedDataMatrix2) <- targets2$Case_Number

save(normalizedDataMatrix2, file = paste(RData,"normalizedDataMatrix.RData",sep=""))
...
NumOfSample <- c(1,44,45,88,89,132)

experimentFactor <- as.factor(substr(colnames(normalizedDataMatrix2), 1, nchar(colnames(normalizedDataMatrix2))-2))
...
if(sampleRelation) {
  ...
  clusterFun(normalizedDataMatrix2, range1=1, range2=ncol(normalizedDataMatrix2), normalized=TRUE,  
             experimentFactor=experimentFactor,clusterOption1=clusterOption1, clusterOption2=clusterOption2,
             plotColors=plotColors, legendColors=legendColors, plotSymbols=plotSymbols, legendSymbols=legendSymbols,
             WIDTH=WIDTH,HEIGHT=HEIGHT,POINTSIZE=12,MAXARRAY=144) 
...
fit<-lmFit(normalizedDataMatrix2, design) # linear model fit for each gene
...
fit3<-lmFit(normalizedDataMatrix2, design3) # linear model fit for each gene
...
if (REUSE){
  load(file = paste(RData,"normalizedDataMatrix.RData",sep=""))
}
...
if (REUSE == FALSE){
  ...
  HighGlucoseData<-normalizedDataMatrix2[,HighGlucoseSamples]  # data in log2 scale
  NoGlucoseData<-normalizedDataMatrix2[,NoGlucoseSamples]      # data in log2 scale
  ...
  rm(deltaData.T, meanDeltaData.T, normalizedDataMatrix, 
